...
1. Call get_my_superpowered_state first. Continue only when assessmentSave.allowed is true. Otherwise stop before start and present the server-returned primary recovery, ordered alternatives, and canonical URLs without guessing, upselling, or attempting a write.
...
- This package does not activate a full local-session capability claim. Require fresh acceptance evidence for the current host and exact helper before any helper-backed read. Without it, disclose the capability limitation and do not claim a full local-session assessment.
